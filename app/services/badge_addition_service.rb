...
=begin
  def call
    current_test_parameters = [@current_test_passage.test.category.name, @current_test_passage.test.title, level_difficulty_string(@current_test_passage.test.level)]
    total_parameters = check_by_parameters(current_test_parameters)
    return @user_badges if total_parameters.any?
